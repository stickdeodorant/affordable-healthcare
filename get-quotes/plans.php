...
$user_phone  = $_SESSION['Primary_Phone'];
$nophone = false;
...
<div class="jumbotron blue-gradient py-5">
    <div class="container text-center">
        <?php
        $falconsSRC = array("Falcons1", "Falcons2", "Falcons3", "Falcons", "Fa", "Fa1", "Fa2", "Fa3");
        if (!in_array($_GET['src'], $falconsSRC)) { ?>
            <div class="stepprogress step-3" style="margin-bottom: 48px;">
        ...
        <?php } ?>
        <h2 style="font-weight:600;font-size:56px;line-height:34px;margin-bottom:calc(10px + 1.5vw);text-transform:capitalize;">Thank you!</h2>
        <p>Your application has been submitted.<br>Want quicker results? Call Now</p>
        <?php if ($_SESSION['typ_phone']) { ?>
            <a class="lg-call-button" href="tel:<?=$phonemin['kobe2']?>" onclick="ga('send', 'event', 'Call Buttons', 'Click', '<?php echo ($_GET['type'] == 'medicare') ? 'medicare' : 'healthcare'; ?>', 'main');"><?=$phone['kobe2']?></a>
        <?php } else if ($_SESSION['campaign'] == 'Sa1') { ?>
            <a class="lg-call-button" href="tel:<?=$phonemin['saints-typ']?>" onclick="ga('send', 'event', 'Call Buttons', 'Click', '<?php echo ($_GET['type'] == 'medicare') ? 'medicare' : 'healthcare'; ?>', 'main');"><?=$phone['saints-typ']?></a>
        <?php } else if ($_SESSION['campaign']) { ?>
            <a class="lg-call-button" href="tel:<?=$phonemin['email']?>" onclick="ga('send', 'event', 'Call Buttons', 'Click', '<?php echo ($_GET['type'] == 'medicare') ? 'medicare' : 'healthcare'; ?>', 'main');"><?=$phone['email']?></a>
        <?php } else if ($_SESSION['fb']) { ?>
            <a class="lg-call-button" href="tel:<?=$phonemin['fb-typ']?>" onclick="ga('send', 'event', 'Call Buttons', 'Click', '<?php echo ($_GET['type'] == 'medicare') ? 'medicare' : 'healthcare'; ?>', 'main');"><?=$phone['fb-typ']?></a>
        <?php } else { ?>
            <?php if (!$nophone) { ?>
...
<?php //include ($_GET['type'] == 'medicare') ? 'inc/pricing/medicare.php' : 'inc/pricing/healthcare.php'; ?>
<style type="text/css">
.modal .phone-number {
    position: absolute;
    background: white;
...
@media (max-width: 375px) {
    ...
    .xxs-none { display: none !important; }
}
</style>
<!-- Modal -->
